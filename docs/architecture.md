...
現在の Orbit は CSR（クライアントサイドレンダリング）のみ。
将来 SSR を導入したとき、このアーキテクチャが壊れないかを検証した結果を残す。

### SSR で変わること

初回アクセス時に、サーバーがデータ取得と HTML 生成を担当する：

```
サーバー（初回アクセス）:
  server.ts の loader → Fetch → Render → HTML + データ
    ↓ HTML + serialized data
クライアント:
  Hydrate → Providers → Layout → Page（State → Transform → Mutate → Re-render）
```

以降のページ遷移は SPA（クライアントナビゲーション）として動く。

### SSR で変わらないこと

- **データフロー** — State → Fetch → Transform → Mutate → Render の順序は同じ
- **ファイル規約** — page / hooks / server / schema の4ファイル構成はそのまま
- **Progressive Decomposition** — 粒度の判断基準は変わらない
- **判断フレームワーク** — 粗く書く → 読めなくなったら分解 → 分解先は規約
...
サーバーでも認証ユーザー等が必要になる。
サーバーで取得 → シリアライズして HTML に埋め込む → クライアントの Providers が引き継ぐ。
Providers の構造自体は壊れない。

**2. hooks.ts の Fetch（useQuery）**

サーバーが loader でデータを取得済みなので、クライアントで二重取得しないようにする。
解法: サーバーが取得したデータを QueryClient のキャッシュに事前注入する。

```tsx
// hooks.ts — CSR でも SSR でも同じコード。変更不要
export function useUser(id: string) {
  return useQuery({
    key: ["users", id],
    fn: ({ signal }) => fetch(`/api/users/${id}`, { signal }),
  })
}
```

useQuery は「キャッシュがあれば使う、なければ fetch する」。
SSR ではサーバーがキャッシュを埋めるだけ。hooks.ts から見た違いはない。

**3. ページ間フロー（invalidate）**

初回以降は SPA として動くため、mutation → invalidate → navigate のフローは CSR と同じ。
SSR が影響するのは「初回アクセス時の Fetch をサーバーでやるか」だけ。

### server.ts の意味が変わる

CSR では server.ts は「将来のため」の規約だった。SSR を入れると文字通りサーバーで実行されるファイルになる。

```
routes/users/[id]/
  page.tsx      ← クライアント（変わらない）
  hooks.ts      ← クライアント（変わらない）
  server.ts     ← サーバーで実行される（← ここだけ変わる）
  schema.ts     ← 両方で使う（変わらない）
```

ファイル規約の設計が SSR を見越していたため、アーキテクチャの変更なしに SSR を導入できる。

### SSR 実装時に必要になるもの

実装はアーキテクチャが実アプリで検証された後に行う。

- サーバービルドパイプライン（server build + client build）
- サーバー関数の実行ランタイム
- データのシリアライズと hydration
- Cloudflare Workers 向けのアダプタ

## server.ts は RPC — loader / action からの脱却

### なぜ loader / action をやめるか

loader / action は Remix が導入した規約で、「ルートに紐づくデータ取得関数」と「ルートに紐づくデータ変更関数」を名前で区別するパターン。

Orbit はこのパターンを採用しない。代わりに **server.ts にただの関数を書く（RPC スタイル）** 。
...
loader / action は「読み取りは loader、書き込みは action」という2つの概念を覚える必要がある。
RPC なら全部ただの関数。覚えることが1つ。

**3. 「隠さない」**

loader はフレームワークが「ルートにアクセスされたら自動で実行する」。この暗黙の実行タイミングを知らないと理解できない。
RPC は hooks.ts から明示的に呼ぶ。呼び出しがコードに書いてある。

**4. ファイル規約が名前規約を不要にする**

Remix や TanStack Router は、コンポーネントと loader が同じファイルに同居するから名前で区別する必要がある。
Orbit は server.ts が独立したファイルとして存在する。ファイル自体が「ここはサーバー処理」と示している。
中の関数に特別な名前規約は要らない。

### 1つの loader では足りない

loader パターンは1ルートに1つの loader。複数のデータが必要な場合、全部1つにまとめるしかない：

```tsx
// loader — 全部まとめる。1つ失敗したら全部失敗する
...
RPC なら各関数が独立する。Progressive Decomposition と相性がいい：

```tsx
// server.ts — 関数が独立。必要なものだけ使う
export async function getUser(id: string) { ... }
...
```tsx
// page.tsx — 目次として読める。各 Fetch が独立
const { data: user } = useUser(id)
const { data: posts } = useUserPosts(user.id)
const { data: permissions } = usePermissions(user.roleId)
```

### 業界のトレンド

loader / action は Remix 時代の設計。新しいフレームワークは RPC / Server Functions に向かっている：

- Next.js: Server Actions（`"use server"` で関数が RPC に）
- SolidStart: Server Functions
- TanStack Start: Server Functions 採用、loader/action から移行中
- tRPC: TypeScript RPC の代表格

Orbit がこの方向に進むのは、トレンドとも一致している。

### server.ts の新しい役割

```
server.ts  → 何ができるか（サーバー上のデータアクセス関数）
hooks.ts   → どう使うか（React との接続: useQuery, useMutation でラップ）
page.tsx   → 何を表示するか（構成: hooks の呼び出しと JSX）
schema.ts  → データの形は何か（型定義とバリデーション）
```
