...
Orbit という、AI と一緒に開発するときの「レール」を作るプロジェクトをやっている。Rails が convention over configuration で人間の生産性を上げたように、AI が迷わず開発できる規約を整えたい。Orbit Router はその一部品だ。

Next.js の App Router、めちゃくちゃ便利だと思う。`app/users/[id]/page.tsx` にファイルを置くだけでルートになる。レイアウトもディレクトリに `layout.tsx` を置けば勝手にネストされる。直感的で、書きやすい。

一方で、Next.js の外でこのディレクトリベースのルーティングを使おうとすると、選択肢が意外とない。

TanStack Router はファイルベースルーティングを提供してくれるけど、あれは `routes/users.tsx` のようにフラットにファイルを並べるスタイルだ。それもシンプルでいいんだけど、個人的には App Router のように **ディレクトリ構造がそのまま URL 構造になる** 方が好みだった。プロジェクトを開いたときにルーティングが一目でわかるし、レイアウトの階層も自然に表現できる。

それなら作ってしまおう、というのが Orbit Router の出発点だ。

もう一つの動機は、フロントエンドのツールチェーンが変わりつつあること。Vite+ (VoidZero) が出てきて、Rolldown + Oxlint + Vitest が統合された新しい開発体験が見えてきた。この波に乗っかる形で、Vite プラグインとしてルーターを作ってみたかった。

**ゴール:**
- App Router 形式のディレクトリベースルーティング
- Vite プラグインとして提供（フレームワーク非依存）
- 必要最低限の軽量さ

今回は Phase 1 として、CSR（クライアントサイドレンダリング）のみの最小ルーターを作った。コア部分はわずか300行。この記事では、その中身を解説していく。
...
export interface RouteEntry {
  ...
  filePath: string;   // index.tsx のフルパス
  layouts: string[];  // layout.tsx のフルパス一覧（外→内の順）
}
```

やっていることはシンプルだ。ディレクトリを歩いて、`index.tsx` を見つけたらルートとして登録。`[id]` のようなブラケット記法は `:id` に変換する。レイアウトは今いるディレクトリから `routes/` のルートまで遡って `layout.tsx` を集める。
...
```js
import Route0 from "/absolute/path/to/routes/index.tsx";
import Layout0 from "/absolute/path/to/routes/layout.tsx";
import Route1 from "/absolute/path/to/routes/about/index.tsx";
...
<RootLayout>        ← routes/layout.tsx
  <UsersLayout>     ← routes/users/layout.tsx
    <UserDetail />  ← routes/users/[id]/index.tsx
  </UsersLayout>
</RootLayout>
```
