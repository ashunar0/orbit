...
export default function NewBookmark() {
  ...
  const handleSubmit = async (data: { url: string; title: string; description: string; tags: string }) => {
    await create(data)
    navigate("/bookmarks")
  }
    ...
    <div className="max-w-xl mx-auto p-4">
      ...
      <Form form={form} onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input {...form.register("url")} placeholder="https://..." className="w-full border rounded px-3 py-1.5 text-sm" />
          {form.fieldError("url") && <p className="text-red-500 text-xs mt-1">{form.fieldError("url")}</p>}
        </div>
        <div>
        ...
        <div>
          ...
          <textarea {...form.register("description")} className="w-full border rounded px-3 py-1.5 text-sm min-h-[60px]" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Tags (comma separated)</label>
          <input {...form.register("tags")} placeholder="react, docs" className="w-full border rounded px-3 py-1.5 text-sm" />
        </div>
